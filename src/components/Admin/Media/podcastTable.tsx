...
const podcasts = Array.from({ length: 50 }, (_, i) => ({
  ...
  thumbnail: `/images/podcast.png`, // Cycle through 8 images
}));
  ...
  return (
    <div className="p-6 bg-white min-h-screen">
      ...
      <div className="flex items-center gap-2 mb-4 text-sm text-gray-600">
        ...
        <span>›</span>
        <span className="font-medium text-gray-800">Podcasts</span>
      </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 border border-gray-300 rounded-full bg-white hover:border-blue-400 transition-colors">
            <svg
              ...
              viewBox="0 0 24 24"
            >
              <rect x="3" y="4" width="18" height="18" rx="2" ry="2" strokeWidth="2" strokeLinecap="round" />
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-blue-600 hover:bg-blue-600">
              <TableHead className="py-3 px-4 font-semibold text-white">Date</TableHead>
              <TableHead className="py-3 px-4 font-semibold text-white">Topic</TableHead>
              <TableHead className="py-3 px-4 font-semibold text-white">Host</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {currentPodcasts.length > 0 ? (
              currentPodcasts.map((podcast) => (
                <TableRow key={podcast.id} className="border-t border-gray-200 hover:bg-gray-50">
                  <TableCell className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div className="text-sm text-gray-700">{podcast.date}</div>
                    </div>
                  </TableCell>
                  <TableCell className="py-3 px-4 font-medium text-gray-900">{podcast.topic}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-700">{podcast.host}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
